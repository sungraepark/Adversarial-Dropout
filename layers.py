...
def mean_only_bn(x, dim, is_training=True, update_batch_stats=True, collections=None, name="bn"):
    params_shape = (dim,)
    ...
    if is_training:
        ...
        with tf.control_dependencies([avg_mean_assign_op]):
            z = (x - mean) 
    else:
        z = (x - avg_mean) 
...
def gl(input_layer, std, seed=None):
    if std!= 0.0:
