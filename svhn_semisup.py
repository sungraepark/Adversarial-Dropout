...
def prepare_dataset():
    ...
    convert_images_and_labels(labeled_train_images,
                              ...
                              os.path.join(dirpath, 'svhn_semisup_labeled_train.tfrecords'))
    convert_images_and_labels(train_images, train_labels,
                              os.path.join(dirpath, 'svhn_semisup_unlabeled_train.tfrecords'))
    convert_images_and_labels(test_images,
                              test_labels,
                              os.path.join(dirpath, 'svhn_semisup_test.tfrecords'))

def one_transformed_input(batch_size=100,
           train=True, shuffle=True, num_epochs=None):

    if train:
        filenames = ['svhn_semisup_labeled_train.tfrecords']
        num_examples = FLAGS.num_labeled_examples
    else:
        filenames = ['svhn_semisup_test.tfrecords']
...
def unlabeled_one_transformed_input(batch_size=100, shuffle=True):
    ...
    filenames = ['svhn_semisup_unlabeled_train.tfrecords']
